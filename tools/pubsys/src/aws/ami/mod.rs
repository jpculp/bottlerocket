...
use crate::aws::{client::build_client_config, parse_arch};
use crate::Args;
...
async fn _run(args: &Args, ami_args: &AmiArgs) -> Result<HashMap<String, Image>> {
    ...
    let arch = ami_args.arch.as_str().to_owned();
    ...
    // register a new one.
    let maybe_id = get_ami_id(&ami_args.name, &arch, &base_region, &base_ec2_client)
        .await
    ...
    for region in regions.iter() {
        ...
        let get_request = get_ami_id(&ami_args.name, &arch, region, ec2_client);
    ...
    let get_responses: Vec<(String, std::result::Result<Option<String>, register::Error>)> =
    ...
    for (region, get_response) in get_responses {
        ...
        if let Some(id) = get_response {
            ...
            amis.insert(region.clone(), Image::new(&id, &ami_args.name));
            continue;
        }
        ...
        let copy_future = ec2_client
            ...
            .set_source_region(Some(base_region.clone()))
            .send();
        ...
        match copy_response {
            Ok(success) => {
                if let Some(image_id) = success.image_id {
                    ...
                    amis.insert(region.clone(), Image::new(&image_id, &ami_args.name));
                } else {
                    saw_error = true;
                    error!(
...
async fn get_account_ids(
    ...
    for (region, response) in responses {
        let response = response.context(error::GetCallerIdentitySnafu { region })?;
